...
import "./Menu.css";
import { mdiDotsHorizontalCircleOutline, mdiCart, mdiBell } from "@mdi/js";
...
const Menu = () => {
  ...
  useEffect(() => {
    ...
    const total = selectedItems.reduce(
      (acc, el) => acc + parseFloat(el.price) * parseInt(el.count),
      0
    );
    ...
    if (!validTables.includes(table)) {
      ...
      try {
        ...
        if (result.success === true) {
          ...
          setSelected("Salad");
          setSelectedItems([]);
        } else alert("There was a problem trying to send your order");
      } catch (error) {
  ...
  const callWaiter = async () => {
    if (!validTables.includes(table)) {
      ...
      alert(`Waiter called at table ${table}`);
    }
  };
      ...
      {showSideMenu ? (
        ...
        <>
          <div className="container">
                ...
                ([categoryName, categoryData]) => {
                  return (
                    <div
                      ...
                      id={categoryName}
                      className={`catCont ${
                        selected === categoryName ? "selectedCat" : ""
                      }`}
                        ...
                        <img
                          ...
                          src={
                            selected === categoryName
                              ? categoryData.icon.replace("FFFFFF", "D4AF37")
                              : categoryData.icon
                          }
                          alt={categoryName}
                        />
            ...
            <div className="items">
              <div className="ringWaiter" onClick={callWaiter}>
                <Icon path={mdiBell} size={1.3} className="ringBell"></Icon>
                <span>Waiter</span>
              </div>
              <img
          ...
          <dialog ref={cartRef}>
            <div>
              <span className="itemsCart">
                <b>{allItems}</b> items in cart
              </span>
              <span className="priceCart">
                Price: <span>{totalPrice} €</span>
              </span>
              <div className="cartIcon">
                <Icon path={mdiCart} size={1.2} onClick={sendOrder}></Icon>
              </div>
            </div>
          </dialog>
        </>
